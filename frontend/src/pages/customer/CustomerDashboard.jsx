...
import { Calendar, MapPin, DollarSign, Clock, Star, MessageSquare, Compass, AlertCircle, X, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
        <div>
          <h1 className="text-3xl font-heading font-extrabold text-slate-900 dark:text-white">Customer Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-dark-900 p-6 rounded-2xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm flex items-center space-x-4">
          <div className="p-3 bg-yellow-100 dark:bg-yellow-950/60 rounded-xl text-yellow-600 dark:text-yellow-400">
            <Clock size={24} />
          </div>
          <div>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Pending Tasks</p>
            <p className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">{pendingBookings}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-dark-900 p-6 rounded-2xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">{completedBookings}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-dark-900 p-6 rounded-2xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">${totalSpent}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {trackingBooking ? (
            <div className="bg-white dark:bg-dark-900 p-6 rounded-3xl border border-slate-200/50 dark:border-slate-800/80 shadow-md space-y-6">
              ...
              <div className="bg-slate-50 dark:bg-dark-950/60 p-4 rounded-2xl flex items-center justify-between border border-slate-150 dark:border-slate-800">
                <div className="flex items-center space-x-3.5">
                  ...
                  />
                  <div>
                    <p className="text-xs font-bold text-slate-400">Assigned Professional</p>
                ...
                <div>
                  <a
                    ...
                    className="bg-primary-50 dark:bg-slate-900/60 text-primary-600 dark:text-primary-400 font-semibold text-xs px-3.5 py-2 rounded-xl border border-primary-500/10 hover:bg-primary-100 transition-all"
                  >
                    Call Provider
                  </a>
                </div>
          ...
          <div className="bg-white dark:bg-dark-900 rounded-3xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="p-8 text-center text-slate-400">
                ...
                No service bookings logged yet. Visit the Home Page to choose a service.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-dark-950/40 border-b border-slate-150 dark:border-slate-800 text-[10px] uppercase font-extrabold tracking-wider text-slate-400">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
                    {bookings.map((booking) => (
                      <tr key={booking._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors">
                        <td className="p-4 pl-6">
                          <p className="font-bold text-slate-800 dark:text-slate-200">
                            {booking.serviceId?.title || 'Unknown Service'}
                          </p>
                          <p className="text-xs text-slate-400 mt-0.5">Category: {booking.serviceId?.category}</p>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold mr-1.5 ${getStatusColor(booking.status)}`}>
                            {booking.status}
                          </span>
                          ...
                          <div className="flex flex-col sm:flex-row items-end sm:items-center justify-end gap-1.5">
                            ...
                            {['pending', 'accepted'].includes(booking.status) && (
                              <button
                                ...
                                className="px-2.5 py-1 text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 hover:bg-red-100 rounded-lg transition-all"
                              >
                                Cancel
                              </button>
                            )}
                            ...
                            {booking.status === 'completed' && booking.paymentStatus === 'pending' && (
                              <button
                                ...
                                className="px-2.5 py-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100 rounded-lg transition-all"
                              >
                                Pay Now
                              </button>
                            )}
                            ...
                            {booking.status === 'completed' && (
                              <button
                                ...
                                className="px-2.5 py-1 text-xs font-bold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-950/30 hover:bg-primary-100 rounded-lg transition-all flex items-center space-x-1"
                              >
        ...
        <div className="space-y-6">
          {/* User profile card */}
          <div className="bg-white dark:bg-dark-900 p-6 rounded-3xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm text-center space-y-4">
            <img
              src={user?.profileImage || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(user?.name || 'C')}`}
              alt={user?.name}
              className="w-20 h-20 rounded-full mx-auto object-cover border border-primary-500/20"
            />
            <div>
              <h3 className="font-heading font-extrabold text-slate-800 dark:text-white text-lg">{user?.name}</h3>
              <p className="text-xs text-slate-400 mt-0.5">{user?.email}</p>
            </div>
            
            <div className="pt-4 border-t border-slate-100 dark:border-slate-850 text-left space-y-3.5 text-xs text-slate-500 dark:text-slate-400">
              <div className="flex items-start space-x-2">
                <Calendar size={14} className="text-slate-400 flex-shrink-0 mt-0.5" />
                <span>Registered: {user ? new Date(user.createdAt).toLocaleDateString() : 'N/A'}</span>
              </div>
              <div className="flex items-start space-x-2">
                <MapPin size={14} className="text-slate-400 flex-shrink-0 mt-0.5" />
                <span>Address: {user?.address || 'Not specified'}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {reviewModalOpen && selectedBooking && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center px-4">
          ...
          <div className="relative bg-white dark:bg-dark-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 max-w-md w-full z-10 space-y-6 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex justify-between items-center">
              <h3 className="font-heading font-bold text-lg text-slate-900 dark:text-white">Review Professional</h3>
              <button onClick={() => setReviewModalOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-white">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-4">
              <div>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Professional details</p>
                <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{selectedBooking.providerId?.name}</p>
                <p className="text-xs text-slate-400">Job: {selectedBooking.serviceId?.title}</p>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Rating Score</label>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Comments & Feedback</label>
                <textarea
                  required
                  rows={4}
                  value={comment}
              ...
              <div className="flex justify-end space-x-2 pt-4">
                <button
                  ...
                  className="px-4 py-2 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-350 rounded-xl text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-primary-600 to-accent-600 text-white rounded-xl text-xs font-bold hover:from-primary-700 hover:to-accent-700 transition-all shadow-md disabled:opacity-50"
                >
