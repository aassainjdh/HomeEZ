...
import { 
  Briefcase, DollarSign, Star, Calendar, Clock, MapPin, 
  AlertTriangle, ShieldCheck, Check, X, ToggleLeft, ToggleRight, Sparkles 
} from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 min-h-screen mesh-gradient-bg scene-3d">
      ...
      <div className="animate-fade-in-up">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">
          Hello, {user?.name}. Manage job requests, update offered services, and track earnings metrics here.
        </p>
      </div>
      ...
      {providerProfile && (
        <div className="animate-fade-in-up">
          {providerProfile.verificationStatus === 'pending' && (
            <div className="bg-yellow-50/80 dark:bg-yellow-950/20 border border-yellow-200/50 dark:border-yellow-900/50 p-5 rounded-2xl flex items-start space-x-3.5 text-yellow-800 dark:text-yellow-400 backdrop-blur-md">
          ...
          {providerProfile.verificationStatus === 'rejected' && (
            <div className="bg-red-50/80 dark:bg-red-950/20 border border-red-200/50 dark:border-red-900/50 p-5 rounded-2xl flex items-start space-x-3.5 text-red-800 dark:text-red-400 backdrop-blur-md">
              <AlertTriangle className="flex-shrink-0 mt-0.5" size={20} />
              <div>
          ...
          {providerProfile.verificationStatus === 'verified' && (
            <div className="bg-emerald-50/80 dark:bg-emerald-950/20 border border-emerald-200/50 dark:border-emerald-900/50 p-5 rounded-2xl flex items-start space-x-3.5 text-emerald-800 dark:text-emerald-400 backdrop-blur-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in-up">
        ...
        <div className="glass-card bg-white/70 dark:bg-dark-900/60 p-6 rounded-3xl border border-slate-200/40 dark:border-slate-850 flex items-center space-x-4 hover:-translate-y-1 transition-transform">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-slate-900 dark:text-white mt-0.5">{completedJobs.length}</p>
          </div>
        </div>
        ...
        <div className="glass-card bg-white/70 dark:bg-dark-900/60 p-6 rounded-3xl border border-slate-200/40 dark:border-slate-850 flex items-center space-x-4 hover:-translate-y-1 transition-transform">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-slate-900 dark:text-white mt-0.5">${totalEarnings}</p>
          </div>
        </div>

        <div className="glass-card bg-white/70 dark:bg-dark-900/60 p-6 rounded-3xl border border-slate-200/40 dark:border-slate-850 flex items-center space-x-4 hover:-translate-y-1 transition-transform">
          <div className="p-3 bg-amber-100 dark:bg-amber-950/60 rounded-2xl text-amber-500 shadow-sm">
            <Star size={22} fill="currentColor" />
          </div>
          <div>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Rating Score</p>
            <p className="text-2xl font-extrabold text-slate-900 dark:text-white mt-0.5">
              {providerProfile?.rating.toFixed(1) || '5.0'}
            </p>
          </div>
        </div>

        <div className="glass-card bg-white/70 dark:bg-dark-900/60 p-6 rounded-3xl border border-slate-200/40 dark:border-slate-850 flex items-center space-x-4 hover:-translate-y-1 transition-transform">
          <div className="p-3 bg-accent-100 dark:bg-accent-950/60 rounded-2xl text-accent-600 dark:text-accent-400 shadow-sm">
            {availability ? <ToggleRight size={22} /> : <ToggleLeft size={22} />}
          </div>
          <div>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Job Availability</p>
            <p className="text-base font-bold text-slate-800 dark:text-slate-200 mt-0.5">
              {availability ? 'Active (Open)' : 'Inactive (Closed)'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in-up">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="glass-panel bg-white/70 dark:bg-dark-900/60 rounded-[2rem] border border-slate-200/40 dark:border-slate-850 shadow-md overflow-hidden">
            <div className="p-6 border-b border-slate-150/40 dark:border-slate-850 flex justify-between items-center bg-slate-50/50 dark:bg-dark-950/20">
              <h3 className="font-heading font-bold text-lg text-slate-900 dark:text-white">New Job Requests</h3>
              <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2.5 py-1 rounded-full dark:bg-yellow-950/40 dark:text-yellow-400 animate-pulse">
            ...
            {loading ? (
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {pendingRequests.map((booking) => (
                  <div key={booking._id} className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-slate-50/30 dark:hover:bg-dark-950/10 transition-colors">
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-bold text-slate-850 dark:text-white">{booking.serviceId?.title}</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-2 flex-shrink-0">
                      <button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl p-2.5 text-xs font-bold transition-all flex items-center space-x-1 shadow-md shadow-emerald-500/10 hover:-translate-y-0.5 active:translate-y-0"
                      >
                        <Check size={14} />
                        <span>Accept</span>
                      </button>
                      <button
                        ...
                        className="bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 border border-red-200/20 rounded-xl p-2.5 text-xs font-bold hover:bg-red-100 dark:hover:bg-red-950 transition-all flex items-center space-x-1 hover:-translate-y-0.5 active:translate-y-0"
                      >
                        <X size={14} />
                        <span>Reject</span>
                      </button>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {activeJobs.map((booking) => (
                  <div key={booking._id} className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-slate-50/30 dark:hover:bg-dark-950/10 transition-colors">
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <span className="text-[10px] font-bold px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-350 rounded uppercase tracking-wider">
                          {booking.serviceId?.category}
                        </span>
                        <span className="text-sm font-bold text-slate-855 dark:text-white">{booking.serviceId?.title}</span>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-500 dark:text-slate-400 font-medium">
                        <p className="flex items-center space-x-1">
                          <Calendar size={12} className="text-slate-400" />
                      ...
                      <p className="text-xs text-slate-400 font-bold">
                        Customer: <span className="text-slate-700 dark:text-slate-300 font-semibold">{booking.customerId?.name}</span> ({booking.customerId?.phone})
                      </p>
                    </div>
                    ...
                    <div className="flex-shrink-0">
                      {booking.status === 'accepted' ? (
                        <button
                          ...
                          className="bg-primary-600 hover:bg-primary-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-md shadow-primary-500/10 hover:-translate-y-0.5 active:translate-y-0"
                        >
                          Start Job
                        </button>
                      ) : (
                        <button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-md shadow-emerald-500/10 hover:-translate-y-0.5 active:translate-y-0"
                        >
                          Complete Job
                        </button>
                      )}
          ...
          <div className="glass-panel bg-white/70 dark:bg-dark-900/60 p-6 rounded-[2rem] border border-slate-200/40 dark:border-slate-850 shadow-md space-y-6">
            ...
            <form onSubmit={handleProfileSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider">Years of Experience</label>
                <input
                  type="number"
                  required
                  min={0}
                  ...
                  className="w-full bg-white/70 dark:bg-dark-950/80 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 text-slate-800 dark:text-white"
                />
              </div>

              {/* Bio/Description */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-455 dark:text-slate-500 uppercase tracking-wider">Bio / Description</label>
                <textarea
                  rows={4}
                  required
                  value={description}
              ...
              {/* Availability Toggle */}
              <div className="flex items-center justify-between p-3.5 bg-white/70 dark:bg-dark-950/60 rounded-xl border border-slate-200/40">
                <div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider block">Offered Services</label>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {categories.map((cat) => (
                    ...
                    >
                      <span>{cat}</span>
                      {selectedServices.includes(cat) && <Check size={12} className="text-primary-500" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-primary-600 to-accent-600 text-white font-bold py-3.5 rounded-xl shadow-md transition-all text-sm hover:from-primary-700 hover:to-accent-700 hover:-translate-y-0.5 active:translate-y-0"
              >
                {updatingProfile ? 'Updating Setup...' : 'Save Parameters'}
              </button>
            </form>
          </div>
        </div>
